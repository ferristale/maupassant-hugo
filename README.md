...
1. Local Search 站内搜索（基于原生 JS 与 RSS）
2. 最近发表文章（默认显示最近 10 篇，支持过滤 `hiddenfromhomepage`）
3. 分类支持（显示分类内文章数量）
4. 标签与标签页支持（文章底部标签样式为 `#标签`）
5. 文章目录 TOC（使用 `<details>` 折叠，仅在有目录时展示）
6. 一键回到顶部（火箭动画）
7. 关键字 SEO 优化（包含 JSON-LD 结构化数据）
8. 深色/浅色主题切换（支持系统主题与本地记忆）
9. 评论系统支持 Disqus / utterances / Waline（点击后再加载评论）
10. 自定义菜单支持（不限个数，自定义排序）
11. 友情链接支持
12. 广告位支持
13. 按日期归档支持
14. GA4 分析统计支持
...
16. 代码高亮、代码行号、代码拷贝
17. sitemap 站点地图
18. RSS 支持，并可自动发现 RSS
19. 自定义 CSS、JS 支持
20. 自定义备案信息支持
21. 自定义图片 CDN 支持
22. Fancybox 图片点击放大（Fancybox v6）
23. 数学公式按需加载（自动检测内容后再加载 MathJax）
24. 搜索、目录等交互采用原生 JS/HTML 实现
...
> 当前仓库未内置 `exampleSite` 目录。请直接在你自己的 Hugo 站点中挂载本主题并配置 `theme = "maupassant"`。

#### 使用 Pixi 安装 Hugo 并测试主题

如果你使用 `pixi` 管理开发环境，在你的 Hugo 站点根目录执行：
...
启动后默认访问 <http://localhost:1313/> 即可预览主题效果。
